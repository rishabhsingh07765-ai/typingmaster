...
function TypingEngine({ text, durationSeconds, onFinish, playSounds, onProgress, forceFinishSignal, hideRestart }) {
  ...
  // Refs mirror the counts so timer/finish callbacks always read the latest
  // value instead of a stale one captured when the effect first ran.
  ...
  // Lets a parent (e.g. the Typing Race game) end this test immediately —
  // used when the bot finishes first, so the player's partial result is
  // captured instead of leaving the engine running forever.
  useEffect(() => {
    if (forceFinishSignal && status === "running") {
  ...
  const finish = (secs) => {
    setStatus("done");
    const c = correctRef.current, inc = incorrectRef.current;
    const { wpm, accuracy } = calcStats(c, inc, secs || elapsed || 1);
    onFinish && onFinish({ wpm, accuracy, correct: c, incorrect: inc, duration: Math.round(secs || elapsed) });
  };
  ...
  const handleChange = (e) => {
    ...
    if (ch === target) { correctRef.current += 1; setCorrectCount(correctRef.current); }
    else { incorrectRef.current += 1; setIncorrectCount(incorrectRef.current); setErrorsAt(prev => new Set(prev).add(idx)); }
    setTyped(val);
    const secs = startRef.current ? (Date.now() - startRef.current) / 1000 : 0;
    if (onProgress) {
      const { wpm, accuracy } = calcStats(correctRef.current, incorrectRef.current, secs || 1);
      onProgress({ pct: Math.min(100, (val.length / text.length) * 100), wpm, accuracy, correct: correctRef.current, incorrect: incorrectRef.current });
    }
  ...
  const restart = () => {
    ...
    correctRef.current = 0; incorrectRef.current = 0;
  ...
  return (
    <div className="engine">
      ...
      {!hideRestart && <button className="btn ghost" onClick={restart}><RotateCcw size={15}/> Restart</button>}
    </div>
  );
}
...
const RACE_DIFFICULTY = {
  easy:   { label: "Easy",   base: 5,  inc: 2.5, cap: 25, min: 5,  max: 20, variance: 0.10, desc: "A gentle pace — great for beginners." },
  medium: { label: "Medium", base: 15, inc: 4,   cap: 50, min: 10, max: 40, variance: 0.15, desc: "A steady, moderately competitive bot." },
  fast:   { label: "Fast",   base: 30, inc: 6,   cap: 70, min: 20, max: 70, variance: 0.20, desc: "A quick bot that keeps you on your toes." },
};

function raceLevelWpm(difficulty, level) {
  const cfg = RACE_DIFFICULTY[difficulty];
  return Math.min(cfg.cap, Math.round(cfg.base + (level - 1) * cfg.inc));
}

function TypingRaceGame() {
  const { recordTest, soundEnabled, profileName } = useApp();
  const [phase, setPhase] = useState("setup"); // setup | countdown | racing | result
  const [difficulty, setDifficulty] = useState("easy");
  const [level, setLevel] = useState(1);
  const [wpmTarget, setWpmTarget] = useState(raceLevelWpm("easy", 1));
  const [countdownVal, setCountdownVal] = useState(3);
  const [text, setText] = useState("");
  const [botPct, setBotPct] = useState(0);
  const [botWpmActual, setBotWpmActual] = useState(0);
  const [player, setPlayer] = useState({ pct: 0, wpm: 0, accuracy: 100, correct: 0, incorrect: 0 });
  const [winner, setWinner] = useState(null); // "you" | "bot"
  const [forceFinish, setForceFinish] = useState(0);
  const [score, setScore] = useState(0);
  ...
  const displayName = profileName && profileName.trim() && profileName !== "Typist" ? profileName : "You";
  const cfg = RACE_DIFFICULTY[difficulty];

  useEffect(() => () => { clearInterval(botIntervalRef.current); clearInterval(countdownRef.current); }, []);

  const pickDifficulty = (id) => { setDifficulty(id); setLevel(1); setWpmTarget(raceLevelWpm(id, 1)); };

  const beginCountdown = () => {
    setText(generateWordStream(28 + level * 2, Date.now() % 100000));
    setBotPct(0); setBotWpmActual(wpmTarget);
    setPlayer({ pct: 0, wpm: 0, accuracy: 100, correct: 0, incorrect: 0 });
    setWinner(null); setForceFinish(0); finishedRef.current = false;
    setCountdownVal(3);
    setPhase("countdown");
    countdownRef.current = setInterval(() => {
      setCountdownVal(v => {
        if (v <= 1) { clearInterval(countdownRef.current); setPhase("racing"); return 0; }
        return v - 1;
      });
    }, 700);
  };
  ...
  useEffect(() => {
    ...
    let current = wpmTarget;
    const totalChars = text.length || 1;
    const reroll = () => {
      const delta = current * cfg.variance;
      current = Math.max(4, Math.min(cfg.max + 5, wpmTarget + (Math.random() * 2 - 1) * delta));
      setBotWpmActual(Math.round(current));
    };
    reroll();
    const rerollTimer = setInterval(reroll, 1200);
    const startTime = Date.now();
    let elapsedSoFar = 0;
    botIntervalRef.current = setInterval(() => {
      const now = Date.now();
      const dt = now - startTime - elapsedSoFar;
      elapsedSoFar = now - startTime;
      const charsPerMs = (current * 5) / 60000;
      setBotPct(p => {
        if (finishedRef.current) return p;
        const next = Math.min(100, p + charsPerMs * dt);
        if (next >= 100 && !finishedRef.current) {
          finishedRef.current = true;
          setWinner("bot");
          setForceFinish(f => f + 1);
        }
        return next;
      });
    }, 120);
    return () => { clearInterval(botIntervalRef.current); clearInterval(rerollTimer); };
  }, [phase]);

  const handlePlayerFinish = ({ wpm, accuracy, correct, incorrect }) => {
    if (!finishedRef.current) {
      finishedRef.current = true;
      setWinner(w => w || "you");
    }
    clearInterval(botIntervalRef.current);
    const finalPlayer = { pct: 100, wpm, accuracy, correct, incorrect };
    setPlayer(finalPlayer);
    const didWin = botPct < 100;
    const gain = Math.max(20, Math.round(wpm * 8 + accuracy * 2 - incorrect * 3 + (didWin ? 150 + level * 20 : 0)));
    setScore(s => s + gain);
    setLastGain(gain);
    recordTest({ wpm, accuracy, duration: Math.round((text.length / 5) / (wpm / 60)) || 10, mode: `Typing Race · Lvl ${level} (${cfg.label})` });
    setPhase("result");
  };
  ...
  const nextLevel = () => {
    const nl = level + 1;
    setLevel(nl);
    setWpmTarget(raceLevelWpm(difficulty, nl));
    beginCountdownFor(nl);
  };

  // small helper so nextLevel uses the *new* level's wpm immediately
  const beginCountdownFor = (nl) => {
    setText(generateWordStream(28 + nl * 2, Date.now() % 100000));
    const nextWpm = raceLevelWpm(difficulty, nl);
    setBotPct(0); setBotWpmActual(nextWpm);
    setPlayer({ pct: 0, wpm: 0, accuracy: 100, correct: 0, incorrect: 0 });
    setWinner(null); setForceFinish(0); finishedRef.current = false;
    setCountdownVal(3);
    setPhase("countdown");
  ...
  const playAgain = () => {
    clearInterval(botIntervalRef.current); clearInterval(countdownRef.current);
    setPhase("setup"); setLevel(1); setScore(0);
    setDifficulty(d => { setWpmTarget(raceLevelWpm(d, 1)); return d; });
  };

  return (
    <div className="card game-card race-game">
      {phase === "setup" && (
        <div className="race-setup">
          <p className="muted">Race {displayName} vs BOT across a short passage. Pick a difficulty, then fine-tune the speed.</p>
          <div className="diff-grid">
            {Object.entries(RACE_DIFFICULTY).map(([id, c]) => (
              <button key={id} className={"diff-card" + (difficulty === id ? " active" : "")} onClick={() => pickDifficulty(id)}>
                <div className="diff-label">{c.label}</div>
                <div className="diff-desc">{c.desc}</div>
                <div className="diff-range">{c.min}–{c.max} WPM</div>
              </button>
            ))}
          </div>
          <div className="wpm-slider-row">
            <label>Bot speed: <strong>{wpmTarget} WPM</strong></label>
            <input type="range" min={5} max={70} step={1}
              value={wpmTarget}
              onChange={e => setWpmTarget(Math.max(5, Math.min(70, Number(e.target.value))))} />
            <div className="wpm-scale"><span>5</span><span>70</span></div>
          </div>
          <button className="btn primary" onClick={beginCountdown}><Play size={15}/> Start Race — Level {level}</button>
          {score > 0 && <div className="muted small">Current session score: {score}</div>}
        </div>
      )}

      {phase === "countdown" && (
        <div className="countdown-overlay">
          <div className="countdown-num">{countdownVal > 0 ? countdownVal : "GO!"}</div>
          <div className="muted small">Level {level} · {cfg.label} · {wpmTarget} WPM bot</div>
        </div>
      )}
      ...
      {(phase === "racing" || phase === "result") && (
        <>
          <div className="race-header">
            <div className="stat-chip accent"><Trophy size={15}/> Score {score}</div>
            <div className="stat-chip"><Award size={15}/> Level {level}</div>
            <div className="stat-chip"><Zap size={15}/> Bot {Math.round(botWpmActual)} WPM</div>
          </div>
          <div className="race-lane"><span title={displayName}>{displayName}</span><div className="race-track"><div className="race-fill you" style={{ width: player.pct + "%" }} /></div></div>
          ...
          {phase === "racing" && (
            <TypingEngine
              ...
              playSounds={soundEnabled}
              hideRestart
              onProgress={(p) => setPlayer(prev => ({ ...prev, ...p }))}
              forceFinishSignal={forceFinish}
          ...
          {phase === "result" && (
            <div className="result-screen">
              <div className="result-emoji">{winner === "you" ? "🏆" : "🤖"}</div>
              <div className="result-title">{winner === "you" ? "YOU WIN" : "BOT WINS"}</div>
              <div className="result-stats-grid">
                <div><span>+{lastGain}</span><label>Score Gained</label></div>
                <div><span>{player.wpm}</span><label>WPM</label></div>
                <div><span>{player.accuracy}%</span><label>Accuracy</label></div>
                <div><span>{player.incorrect}</span><label>Errors</label></div>
                <div><span>{level}</span><label>Level</label></div>
                <div><span>{Math.round(botWpmActual)}</span><label>Bot WPM</label></div>
              </div>
              <div className="result-actions">
                <button className="btn primary" onClick={nextLevel}><ChevronRight size={15}/> Next Level</button>
                <button className="btn ghost" onClick={playAgain}><RotateCcw size={15}/> Play Again</button>
              </div>
            </div>
          )}
        </>
...
.race-game { max-width:100%; overflow-x:hidden; }
.race-lane { display:flex; align-items:center; gap:10px; font-size:12.5px; color:#8792A6; max-width:100%; }
.race-lane span { width:70px; flex-shrink:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; font-weight:600; }
.race-track { flex:1; min-width:0; height:10px; background:#1A2233; border-radius:6px; overflow:hidden; }
.race-fill { height:100%; border-radius:6px; transition:width .12s linear; }
.race-fill.you { background:#5EEAD4; }
.race-fill.bot { background:#F5B942; }

.race-setup { display:flex; flex-direction:column; gap:16px; }
.diff-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(150px,1fr)); gap:10px; }
.diff-card { background:#0E1420; border:1px solid #1E2635; border-radius:12px; padding:14px; text-align:left; cursor:pointer; color:inherit; display:flex; flex-direction:column; gap:5px; }
.diff-card:hover { border-color:#5EEAD4; }
.diff-card.active { border-color:#5EEAD4; background:#0E241F; }
.diff-label { font-weight:700; font-size:14px; }
.diff-desc { font-size:11.5px; color:#8792A6; line-height:1.4; }
.diff-range { font-size:11px; color:#5EEAD4; font-weight:600; margin-top:2px; }
.wpm-slider-row { display:flex; flex-direction:column; gap:6px; background:#0E1420; border:1px solid #1E2635; border-radius:12px; padding:14px; }
.wpm-slider-row label { font-size:12.5px; color:#9AA7BD; }
.wpm-slider-row label strong { color:#5EEAD4; }
.wpm-slider-row input[type=range] { width:100%; }
.wpm-scale { display:flex; justify-content:space-between; font-size:10.5px; color:#5B6678; }

.countdown-overlay { display:flex; flex-direction:column; align-items:center; justify-content:center; gap:8px; padding:50px 10px; }
.countdown-num { font-size:56px; font-weight:800; color:#5EEAD4; line-height:1; }

.race-header { display:flex; gap:10px; flex-wrap:wrap; }

.result-screen { text-align:center; display:flex; flex-direction:column; align-items:center; gap:14px; padding:10px 0 4px; }
.result-emoji { font-size:44px; line-height:1; }
.result-title { font-size:20px; font-weight:800; letter-spacing:.02em; }
.result-stats-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:14px; width:100%; max-width:420px; }
.result-stats-grid span { display:block; font-size:19px; font-weight:800; }
.result-stats-grid label { font-size:10.5px; color:#8792A6; }
...
@media (max-width:860px) {
  ...
  .race-lane span { width:52px; font-size:11.5px; }
